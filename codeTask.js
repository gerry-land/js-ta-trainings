...
function calculateMenAverageAge(people, century) {
  const men = people.filter(person => person.sex === 'm' ? true : false);

  if (century) {
    const menByCentury = men.filter(person => {
      return (Math.floor(person.born / 100) === century || Math.floor(person.died / 100) === century) ? true : false;
    });

    // wrong answer
    return (
      menByCentury.reduce((averageAge, person) => {
        return averageAge + Math.ceil(person.died / 100);
      }, 0)
    );
  };

  return (
    men.reduce((averageAge, person) => {
      return averageAge + (person.died - person.born) / men.length;
    }, 0)
  );
}
...
function calculateWomenAverageAge(people, withChildren) {
  const women = people.filter(person => person.sex === 'f' ? true : false);

  if (withChildren) {
    const womenWithChildren = people.filter(person => {
      return women.some((child) => child.mother === person.name);
    });

    // wrong answer
    return (
      womenWithChildren.reduce((averageAge, person) => {
        return  averageAge + (person.died - person.born) / womenWithChildren.length;
      }, 0)
    )
  }

  return (
    women.reduce((averageAge, person) => {
      return  averageAge + (person.died - person.born) / women.length;
    }, 0)
  )
}
...
function calculateAverageAgeDiff(people, onlyWithSon) {
  ...
  const childs = people.filter(person => {
    if (people.some(mother => mother.name === person.mother)) {
      return onlyWithSon ? person.sex === 'm' : true;
    }
  });

  return (
    childs.reduce((averageAge, person) => {
      const mother = people.find(mother => mother.name === person.mother);
      const motherBornYear = mother.born;
      const childBornYear = person.born;
      return averageAge + (childBornYear - motherBornYear) / childs.length;
    }, 0) 
  );
}
